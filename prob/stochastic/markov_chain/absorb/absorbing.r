...
init <- c(1, rep(0, 100))
## simulate the game
sim_markov(init, P, 50, 0:100)

sim_list <- replicate(1000, which(sim_markov(init, P, 500, 0:100) == 100)[1])
mean(sim_list)

# fundamental matrix
f <- solve(diag(100) - P[1:100, 1:100])
## The expected number of steps from transient state i until the chain is absorbed in some absorbing state
f %*% rep(1, 100)
## probability that the chain started in transient state i is absorbed in state j
f %*% P[1:100, 101]
